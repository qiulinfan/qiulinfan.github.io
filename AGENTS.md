...
- The reusable knowledge graph engine is pinned as the `vendor/kgdistiller` Git submodule; personal sources, registry, graph snapshots, and site integration remain in this repository.
- Run `git submodule update --init --recursive` before knowledge commands in a fresh checkout.
