...
> 2026-08-12 架构更新：本文中的 submodule、qlblog-owned kg Skills 与
> `kgdistiller-update` 是已经被替代的历史设计。现在
> [kgdistiller](https://github.com/qiulinfan/kgdistiller) 是自闭合产品仓库，独立维护
> 引擎、论文/笔记 Skills、预制 agents、工作流、测试与 linker；qlblog 只保留个人
> instance authority、采用后的静态 export、前端与自动部署。产品开发工作树直接链接
> 到本地 Codex，网站采用版本则记录在 export manifest 中。现行 qlblog 边界见
> [`knowledge/SPEC.md`](../../../knowledge/SPEC.md) 与
> [`knowledge/WORKFLOW.md`](../../../knowledge/WORKFLOW.md)。以下正文继续原样保留，
> 仅作为需求演进记录。
...
> 快照；当前流程没有论文 ingest 分支。该说明同样属于历史阶段，现行产品工作流以
> kgdistiller 仓库为准。
