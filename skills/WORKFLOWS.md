...
## Codex Skills 委派给 DeepSeek Agents

```mermaid
flowchart LR
    Req["用户任务"] --> Pick["选择一个或多个 Codex Skills"]
    Pick --> Stage["复制到 Claude Code 项目"]
    Stage --> Shape{"任务拓扑"}
    Shape -->|内聚任务| One["DeepSeek V4 Flash Worker"]
    Shape -->|独立或分阶段任务| Lead["DeepSeek V4 Flash Coordinator"]
...
[`run-workflow-with-agents`](#skill-run-workflow-with-agents) 默认从 Codex
已发现的 skills 解析用户要求的一个或多个能力，物理注入 Claude Code，并统一
使用 DeepSeek V4 Flash。内聚任务由单 worker 完成；可拆分任务由 coordinator
...
flowchart LR
    ...
    Copies --> T1["DeepSeek V4 Flash Trial 1"]
    Copies --> T2["DeepSeek V4 Flash Trial N"]
...
skill，不承接生产交付。单次 smoke、回归和负向测试使用一个一次性 fixture；
