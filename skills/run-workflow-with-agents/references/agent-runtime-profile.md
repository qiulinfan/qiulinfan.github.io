...
The production runner accepts a stable `--workflow` name and resolves every
preloaded Skill against it. Missing routes never cause a question. A route to a
...
agent-selection prompt. Heterogeneous routes remain cached, but the current
single-session coordinator rejects them before staging because it cannot mix
agent products inside one runtime session.
