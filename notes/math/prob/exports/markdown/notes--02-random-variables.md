...
![正、负与零协方差对应的线性趋势](.assets/notes--fig-prob-02-random-variables-diagram-01.svg){#fig-prob-02-random-variables-diagram-01 alt="Three scatterplots with positive, negative, and zero linear covariance trends."}
...
- covariance 只捕捉 linear relationship, 即趋势是否呈直线, 不过不能捕捉 nonlinear relationship. 之后在学习 independence 的时候, 我们会知道即便 covariance 为 0, $X$ 和 $Y$ 仍然可能是 dependent 的. 比如, 如果 $(X,Y)$ 的联合分布 as a graph 是个圆圈, 那么 covariance 就是 0, 但显然这两个变量之间是有关系的.
  ...
  ![零协方差但不独立的圆周分布](.assets/notes--fig-prob-02-random-variables-diagram-02.svg){#fig-prob-02-random-variables-diagram-02 alt="Points on a circle show dependent variables whose covariance is zero."}
...
![参数 $\lambda = 3$ 的 Poisson 概率质量函数](.assets/notes--fig-prob-02-random-variables-diagram-03.svg){#fig-prob-02-random-variables-diagram-03 alt="A stem plot of the Poisson probability mass function with lambda equal to 3."}
...
![区间 $\left\lbrack {a,b} \right\rbrack$ 上均匀分布的概率密度函数](.assets/notes--fig-prob-02-random-variables-diagram-04.svg){#fig-prob-02-random-variables-diagram-04 alt="The uniform density is constant at 1/(b-a) between a and b and zero outside."}

![区间 $\left\lbrack {a,b} \right\rbrack$ 上均匀分布的累积分布函数](.assets/notes--fig-prob-02-random-variables-diagram-05.svg){#fig-prob-02-random-variables-diagram-05 alt="The uniform cumulative distribution rises linearly from zero at a to one at b."}
:::
...
![指数分布的概率密度函数](.assets/notes--fig-prob-02-random-variables-diagram-06.svg){#fig-prob-02-random-variables-diagram-06 alt="The exponential density decreases from lambda toward zero as x increases."}

![指数分布的累积分布函数](.assets/notes--fig-prob-02-random-variables-diagram-07.svg){#fig-prob-02-random-variables-diagram-07 alt="The exponential cumulative distribution rises from zero toward one."}
...
![固定 $\lambda = 1$ 时不同 shape 参数的 Gamma 密度](.assets/notes--fig-prob-02-random-variables-diagram-08.svg){#fig-prob-02-random-variables-diagram-08 alt="Gamma density curves for alpha 1, 2, 3, and 5 with lambda fixed at 1."}

![固定 $\lambda = 1$ 时不同 shape 参数的 Gamma 分布函数](.assets/notes--fig-prob-02-random-variables-diagram-09.svg){#fig-prob-02-random-variables-diagram-09 alt="Gamma cumulative distribution curves for alpha 1, 2, 3, and 5 with lambda fixed at 1."}
