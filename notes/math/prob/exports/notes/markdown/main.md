...
我们可以将问题转化为: 从 $n$ 个 **distinct objects** 中选取 $k$ 个的 permutations 的数量, 然后再除去重复的 permutations. 而, 从 $n$ 个 **distinct objects** 中选取 $k$ 个的 permutations 的数量为:
...
这里的 $\sum_{i < j}$ 等 index 意思就是**考虑所有可能的 combinations**, 不考虑顺序, 和下标的顺序没有关系. 比如一共有三个集合 $A_{1},A_{2},A_{3}$, 那么 $\left. \sum_{i < j} \middle| A_{i} \cap A_{j}| \right.$ 就是 $A_{1} \cap A_{2},A_{1} \cap A_{3},A_{2} \cap A_{3}$ 的并集.\
...
- 一个 set $\Omega$ 的一个 $\sigma$-algebra 就是一个包含空集的子集簇, 其 **closed under countable unions and complements**. (如果只 closed under finite unions, 则只称为一个 algebra.)
...
- 一个 measure on a measurable space $(\Omega,\mathcal{F})$ 就是一个从 $\mathcal{F}\rightarrow{\mathbb{R}}$ 的函数, 满足 **countable additivity**.

- measure 的性质: **(1)non-negativity, (2) monotonicity, (3) countable subadditivity, (4) inclusion-exclusion principle (上节已讲), (5) continuity from below and above.** 第 (5) 条具体即: For measurable sets $A_{1} \subseteq A_{2} \subseteq \cdots \subseteq A_{n} \subseteq \cdots$, then $\lim_{n\rightarrow\infty}{\mathbb{P}}(A_{n}) = {\mathbb{P}}(\bigcup_{n = 1}^{\infty}A_{n})$.\
...
![两人在一小时内到达且等待不超过十分钟的事件区域](main.assets/fig-prob-01-combinatorics-prob-space-diagram-01.svg){#fig-prob-01-combinatorics-prob-space-diagram-01 alt="A 60-by-60 arrival-time square with the band abs(x-y) <= 10 shaded."}
...
这个定义里没有指明, 给定一个 event $B \in \mathcal{F}$, 如何去确定 $\mathcal{G}$ 以及 ${\mathbb{P}}(A \mid \mathcal{G})$. 虽然这是一个抽象的存在性[ ]{style="white-space: pre-wrap"} 唯一性定义, 但是标准的做法是:
...
Further issue: **Borel-Kolmogorov paradox**.\
即便是这个定义, 也会出现问题.\
...
将它推广到多个事件: 即**任意一个事件的发生与否都不影响其他事件发生的概率**.
:::::
...
意思是: 事件 $A$ 在 $B$ 这个 set 中占据的 measure 比例, 完全等于事件 $A$ 在整个空间 $\Omega$ 中占据的 measure 比例; 也就是说, **事件 $B$ 的发生并没有改变事件 $A$ 在空间中的 measure 分布.**
...
原本, 一个 prob space 只能表示: **有多少个 outcomes, 每个 outcome 的概率是多少** (对于无限的 sample space, 一些 outcomes 组合, 即事件, 的概率是多少).

而 random variable 就是**给这些 outcomes 以 weights**: 比如

::: {#ex-02-random-variables-example-001 .example concepts="example-001"}
**Example**
...
**Question**: 为什么对于 assigning weights 的行为, 我们一定要求它是一个 (Borel) measurable function 呢? 普通的给 outcomes 以 weights 的行为 (任意一个 function $f:\Omega\rightarrow{\mathbb{R}}$) 不行吗?

**Answer**: 因为我们想要知道 $X$ 的值在一个范围 $B$, 比如 $\lbrack 1,6\rbrack$ 之内的 probability (which is defined on 原本的 prob measure $\mathbb{P}$ 上的) 是多少. 而只有每一个这样的范围集合的 preimage $X^{- 1}(B)$ 都是 $\mathcal{F}$ 中的事件, 我们才能够通过 $\mathbb{P}$ 来计算这个 probability; that is, 我们需要这个 $X$ 是一个 measurable function.
...
- 统计学中, 我们有一堆样本, **每一个都是随机采样得到的 (即一个 random variable $X_{i}$)**. 因而在观测之前, 我们 把它看作一个 random variable $X_{i}$, 然后在观测之后取到它的一个实际值 $x_{i}$.

  通常我们假设所有样本都是 **independent and identically distributed (i.i.d., 独立同分布)** 的, 即所有的 $X_{1},X_{2},\ldots,X_{n}$ 都是 independent 的, 并且服从同一个 distribution (discuss later...).
...
这个 $\sigma(X)$ 也是让 $X$ 成为一个 measurable function (即 random variable) 的最小的 $\sigma$-algebra. 它的意义是: **当我们仅仅观察到 $X$ 的取值时, 我们所掌握的关于 underlying sample space $\Omega$ 的全部 \"information\".**

:::: remark
**Remark**

我们知道, **一个 $\sigma$-algebra $\mathcal{F}$ 可以看作是一个 information structure**: 它是对 underlying sample space $\Omega$ 进行划分的一个工具.\
...
而一个 random variable $X$ 可能会把多个 outcomes 映射到同一个值上, 也即 把多个 events 映射到同一个值上, 从而把这些 events 区分不开来. 因而, **random variable $X$ generate 的 $\sigma$-algebra $\sigma(X)$ 就可能是一个更粗的划分**. 当我们取这些 events 所映射到的同一个值的 preimage 的时候, 取到的是 它们的 union, 也就是它们中最大的一个.

比如

::: {#ex-02-random-variables-example-002 .example concepts="example-002"}
**Example**
...
因为 $1,3,5$ 这些事件都被映射到了同一个值 $0$ 上, 即 $\left\{ 1 \right\},\left\{ 3 \right\},\left\{ 5 \right\},\left\{ {1,3} \right\}$ 这些事件都被 $\left\{ {1,3,5} \right\}$ 这个 事件吸收了, 它们在这个 random variable $X$ 的映射下是区分不开的. 这就代表了 这个 random variable 所蕴含的信息颗粒度: **我们只能区分掷出的点数是奇数还是偶数, 但是无法区分具体的点数是多少.**\
至此我们知道: **一个 random variable $X$ 的 generate 的 $\sigma$-algebra $\sigma(X)$ 表示了这个 random variable 对 underlying sample space $\Omega$ 所能区分的 information granularity (信息颗粒度)**.\
:::
::::
...
就是说, **$X$ 的 range a.s. 是 countable 的** (允许 null set 上 的值不在这个 countable set 里, 但是这是 measure theory 下的概念, 所以这些都可以忽略不计).
:::
...
这是 uniform continuity 的一个加强版本. uniform continuity 对应的是 $n = 1$ 的情况, 即: **任意的区间上, 只要长度足够小, 就能保证 $F_{X}$ 在这个区间上的增量足够**小; 而 absolutely continuity 更严格: **即便是任意数量的分散的区间, 只要总长度足够小, 就能保证 $F_{X}$ 在这些 interval 上的增量的总和足够小.**

这个定义有点麻烦, 不过存在等价的条件. 为此我们要 recall 一系列定义与结论, 详细请见: [notes on measure theory, module 12: differentiation on $\mathbb{R}$](https://qiulinfan.github.io/mathnotes-measure_theory/12-differentiation_on_real_spaces.html):

- 给定一个 function $F:{\mathbb{R}}\rightarrow{\mathbb{C}}$, 我们定义它的 **total variation function** $V_{F}:{\mathbb{R}}\rightarrow\lbrack 0, + \infty\rbrack$ 为:
...
- **任意的 $F \in NBV$ 都 $m$-a.e. differentiable, 且导数 $F' \in L^{1}(m)$**, 并且以下三个条件等价:
...
其次, 由 [Theorem 2.4](#thm-02-random-variables-distribution-function) 我们知道, $F_{X}$ 是 right-continuous 的, 且 $F_{X}( - \infty) = 0$. 因而 $F_{X} \in NBV$.
:::
...
- $X$ 是一个 continuous random variable 当且 仅当 ${\mathbb{P}}^{X} \ll m$, 即当且仅当存在一个函数 $f_{X}:{\mathbb{R}}\rightarrow\lbrack 0,\infty)$, 使得 使得对于任意 Borel set $A \subseteq {\mathbb{R}}$, 有
  ...
  - **distribution 对于 Lebesgue measure 的 Radon-Nikodym derivative**: $f_{X} = \frac{d{\mathbb{P}}^{X}}{dm}$.

  - **cdf $F_{X}$ 的 $m$-a.e.导数 $f_{X} = F_{X}'$.**

  我们也称这个 $f_{X}$ 是 continuous random variable $X$ 的 **probability density function (pdf).**
:::
...
- **任意的 random variable** 的 cdf $F_{X}$ 都是一个 $NBV$ function, 因而**一定 a.e. 可导**. 但是它的导数 $F_{X}'$ 的积分不一定返回原函数.

- **如果一个 $m$-a.e. 导数 $F_{X}'$ 满足 $F_{X}(x) = \int_{- \infty}^{x}F_{X}'(t) dt$ 对所有 $x \in {\mathbb{R}}$ 成立, 即导数的积分返回原函数, 那么 $X$ 就是一个 continuous random variable**, 而我们定义 $f_{X} := F_{X}'$ 为 $X$ 的 probability density function.
...
但是我们现在知道, 这个定义是错的. 首先, 标准定义中的 absolutely continuous 要强于 continuous (它能推导出 a.e. differentiable); 而且, **这个更强的定义是 necessary 的**.
...
由于其 uncountability, 对于任意单点 $x$ 都有 ${\mathbb{P}}(X = x) = 0$, **因而 $F_{X}$ 在 $\mathbb{R}$ 上处处连续**; 并且容易证明: **$F_{X}$ 在 $x$ 处的导数 $F_{X}'(x) = 0$**. (因为对于 $C^{c}$ 中的任意被挖掉的区间, $F_{X}$ 在这个区间上是 constant 的).

然而, $F_{X}' = 0$ 却**不满足 FTC of Lebesgue integral**. 比如反例: $F_{X}(1) = 1$, 但是
...
最后我们会谈一谈**$L^{2}(\Omega,\mathcal{F},{\mathbb{P}})$ 空间**: all square-integrable random variables; 以及它的一个重要 subspace $L_{0}^{2}(\Omega,\mathcal{F},{\mathbb{P}})$: 所有 square-integrable 的 zero-centered random variables. 它们都是由 random variables 组成的 Hilbert space, 并且在其中, expectation, variance, covariance 都会得到一个非常自然的 interpretation, 展现这个空间的几何结构.

### expectation and variance 的 definition 和计算方法

随机变量的 **expectation** 是它 w.r.t. 它所在概率空间 prob measure $\mathbb{P}$ 的积分, 表示它的值的 prob-weighted average;
...
- **linearity of expectation**: ${\mathbb{E}}\lbrack aX + bY\rbrack = a{\mathbb{E}}\lbrack X\rbrack + b{\mathbb{E}}\lbrack Y\rbrack$ for any $a,b \in {\mathbb{R}}$.

- **monotonicity of expectation**: 如果 $X \leq Y$ a.s., 那么 ${\mathbb{E}}\lbrack X\rbrack \leq {\mathbb{E}}\lbrack Y\rbrack$.

- **absolute value**: $\left. {\mathbb{E}}\lbrack X\rbrack \leq {\mathbb{E}}\lbrack \middle| X \middle| \rbrack \right.$
:::
...
### $L_{0}^{2}(\Omega,\mathcal{F},{\mathbb{P}})$ 与 covariance and correlation as inner product and cosine
...
我们知道, 这是一个 **Hilbert space**, 即一个 complete inner product space, 其中
...
![正、负与零协方差对应的线性趋势](main.assets/fig-prob-02-random-variables-diagram-01.svg){#fig-prob-02-random-variables-diagram-01 alt="Three scatterplots with positive, negative, and zero linear covariance trends."}
...
- covariance 只捕捉 linear relationship, 即趋势是否呈直线, 不过不能捕捉 nonlinear relationship. 之后在学习 independence 的时候, 我们会知道即便 covariance 为 0, $X$ 和 $Y$ 仍然可能是 dependent 的. 比如, 如果 $(X,Y)$ 的联合分布 as a graph 是个圆圈, 那么 covariance 就是 0, 但显然这两个变量之间是有关系的.
  ...
  ![零协方差但不独立的圆周分布](main.assets/fig-prob-02-random-variables-diagram-02.svg){#fig-prob-02-random-variables-diagram-02 alt="Points on a circle show dependent variables whose covariance is zero."}
...
称 $X:\Omega\rightarrow\left\{ {0,1} \right\}$ 这个 random variable 为一个 **Bernoulli random variable**, 并称 $X$ 的 distribution $P^{X}$ 为一个 **Bernoulli distribution**, 写作
...
::: {#prop-02-random-variables-proposition-010 .proposition concepts="proposition-010"}
**Proposition**
...
我们称 $S$ 的 distribution $P^{S}$ 为一个 **Binomial distribution**, 写作
...
::: {#prop-02-random-variables-proposition-011 .proposition concepts="proposition-011"}
**Proposition**
...
我们称 $T$ 的 distribution $P^{T}$ 为一个 **Geometric distribution**, 写作
...
::: {#prop-02-random-variables-proposition-012 .proposition concepts="proposition-012"}
**Proposition**
...
我们称 $T_{r}$ 的 distribution $P^{T_{r}}$ 为一个 **Negative Binomial distribution**, 写作
...
我们称 $X$ 的 distribution $P^{X}$ 为一个 **Poisson distribution**, 写作

$$X \sim \text{Poi}(\lambda),\quad\lambda > 0$$

以 $\lambda = 3$ 为例画出 pmf 示意图:

![参数 $\lambda = 3$ 的 Poisson 概率质量函数](main.assets/fig-prob-02-random-variables-diagram-03.svg){#fig-prob-02-random-variables-diagram-03 alt="A stem plot of the Poisson probability mass function with lambda equal to 3."}
...
因而 Poisson distribution 的意义本质上是: **给定对一个事件在固定窗口下出现的平均次数的观测(估计), 那么这个事件在这个窗口内出现 $n$ 次 ($n = 0,1,2,\cdots$)的概率是多少?**\
所以 Poisson 分布, 即是**根据一个事件在给定 windows 内发生次数的 expectation, 来估计这个事件在该 windows 内发生次数的分布的标准方法.**\
:::
...
::: {#prop-02-random-variables-proposition-013 .proposition concepts="proposition-013"}
**Proposition**
...
::: {#prop-02-random-variables-proposition-014 .proposition concepts="proposition-014"}
**Proposition**
...
![区间 $\left\lbrack {a,b} \right\rbrack$ 上均匀分布的概率密度函数](main.assets/fig-prob-02-random-variables-diagram-04.svg){#fig-prob-02-random-variables-diagram-04 alt="The uniform density is constant at 1/(b-a) between a and b and zero outside."}

![区间 $\left\lbrack {a,b} \right\rbrack$ 上均匀分布的累积分布函数](main.assets/fig-prob-02-random-variables-diagram-05.svg){#fig-prob-02-random-variables-diagram-05 alt="The uniform cumulative distribution rises linearly from zero at a to one at b."}
:::
...
![指数分布的概率密度函数](main.assets/fig-prob-02-random-variables-diagram-06.svg){#fig-prob-02-random-variables-diagram-06 alt="The exponential density decreases from lambda toward zero as x increases."}

![指数分布的累积分布函数](main.assets/fig-prob-02-random-variables-diagram-07.svg){#fig-prob-02-random-variables-diagram-07 alt="The exponential cumulative distribution rises from zero toward one."}
...
::: {#ex-02-random-variables-example-011 .example concepts="example-011"}
**Example**
...
那么我们称 $X$ 服从 **$\Gamma$ distribution**, 写作
...
Gamma distribution 即多个 i.i.d. exponential distribution 的 sum; 而我们知道 exponential distribution 是 geometric distribution 的 continuous analogue; 因而 **Gamma distribution 也是 negative binomial distribution 的 continuous analogue.**

考虑固定 rate parameter $\lambda = 1$，不同 shape parameter $\alpha$ 的 Gamma distribution：

![固定 $\lambda = 1$ 时不同 shape 参数的 Gamma 密度](main.assets/fig-prob-02-random-variables-diagram-08.svg){#fig-prob-02-random-variables-diagram-08 alt="Gamma density curves for alpha 1, 2, 3, and 5 with lambda fixed at 1."}

![固定 $\lambda = 1$ 时不同 shape 参数的 Gamma 分布函数](main.assets/fig-prob-02-random-variables-diagram-09.svg){#fig-prob-02-random-variables-diagram-09 alt="Gamma cumulative distribution curves for alpha 1, 2, 3, and 5 with lambda fixed at 1."}
...
::: {#def-02-random-variables-definition-017 .definition concepts="definition-017"}
**Definition**

我们称 $X$ is **normally distributed with parameter $\mu$ and $\sigma$**, if the pdf of $X$ is given by:
...
被称为 **standard normal distribution.**
:::
...
::: {#ex-02-random-variables-example-012 .example concepts="example-012"}
**Example**
...
- 由于 $X$ 是一个 continuous random variable, ${\mathbb{P}}(X = 1/2) = 0$.
  ...
  ![联合分布 support 中满足 $X + Y \leq \frac{3}{2}$ 的积分区域](main.assets/fig-prob-03-joint-conditional-distribution-diagram-01.svg){#fig-prob-03-joint-conditional-distribution-diagram-01 alt="The shaded portion of the support lies below x+y=3/2, split at x=1/2."}
...
![圆周关系说明零协方差不蕴含独立性](main.assets/fig-prob-03-joint-conditional-distribution-diagram-02.svg){#fig-prob-03-joint-conditional-distribution-diagram-02 alt="Points constrained to a circle have zero covariance but remain dependent."}
...
![独立连续随机变量的矩形 support 与等形横截面](main.assets/fig-prob-03-joint-conditional-distribution-diagram-03.svg){#fig-prob-03-joint-conditional-distribution-diagram-03 alt="A rectangular joint support whose vertical cross-sections share one shape at different scales."}
...
![密度曲线在阈值 $t$ 右侧的尾部概率](main.assets/fig-prob-04-lln-diagram-01.svg){#fig-prob-04-lln-diagram-01 alt="A density curve with the tail to the right of threshold t shaded as P(X >= t)."}
:::
