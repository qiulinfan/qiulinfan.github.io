...
\begin{definition}{\protect\phantomsection\label{kn-random-vector}{\textbf{random vector}}}

对于 prob space \((\Omega,\mathcal{F},P)\), 一个 function \(\mathbf{X}:\Omega\rightarrow{\mathbb{R}}^{n}\) 如果是一个 \((\mathcal{F},\mathcal{B}({\mathbb{R}}^{n}))\)-measurable function (即 \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-mathcal-m-mathcal-n-measurable-function}{\((\mathcal{M},\mathcal{N})\)-measurable function} 在这两个 measurable spaces 上的情形), 则 称它为一个 \(n\)-dimensional \href{https://qiulinfan.github.io/notes/math/probability/\#kn-random-variable}{random variable}, 或者 \(n\)-dimensional random vector.
...
\begin{proposition}{\protect\phantomsection\label{kn-n-random-variable-vector-random-vector}{\textbf{由
\(n\) 个 random variable 构成的 vector 是一个 random vector}}}
...
\begin{proposition}{\protect\phantomsection\label{kn-random-vector-random-variable}{\textbf{一个
random vector 的每个分量都是一个 random variable}}}
...
\begin{definition}{\protect\phantomsection\label{kn-joint-distribution-and-joint-cdf}{\textbf{joint
distribution}} and
\protect\phantomsection\label{kn-joint-cdf}{\textbf{joint cdf}}}

令 \(X_{1},\cdots,X_{n}\) 为 RV from the same prob space. 即 \(\mathbf{X} := (X_{1},\cdots,X_{n})\) 是一个 random vector. 下面的两个对象分别将 \href{https://qiulinfan.github.io/notes/math/probability/\#kn-probability-distribution}{probability distribution} 和 \href{https://qiulinfan.github.io/notes/math/probability/\#kn-distribution-function-cumulative-distribution-function-cdf}{distribution function (也称 \textbf{cumulative distribution function, cdf})} 推广到 random vector.
...
\begin{definition}{\protect\phantomsection\label{kn-marginal-distribution}{\textbf{marginal
distribution}}}
...
\begin{proposition}{\protect\phantomsection\label{kn-marginal-distribution-from-joint-limits}{\textbf{通过
joint distribution 的极限得到 marginal distribution}}}
...
recall: \href{https://qiulinfan.github.io/notes/math/probability/\#kn-continuous-random-variable}{continuous random variable} \(X\) 的 cdf 是 absolutely continuous 的. 这个条件也等价于, 存在一个函数 \(f_{X}:{\mathbb{R}}\rightarrow\lbrack 0,\infty)\) 使得

\[F_{X}(x) = \int_{- \infty}^{x}f_{X}(t)\, dt\]

这个定义可以 generalize 到 random vector 上.

\begin{definition}{\protect\phantomsection\label{kn-continuous-random-vector-continuous-joint-cdf}{\textbf{continuous
random vector}} 和
\protect\phantomsection\label{kn-continuous-joint-cdf}{\textbf{continuous
joint cdf}}}

对于 random vector \(\mathbf{X} = (X_{1},\cdots,X_{n})^{T}\) 如果 \({\mathbb{P}}^{\mathbf{X}} \ll \lambda^{n}\), 即它满足 \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-absolute-continuity-of-signed-measures}{absolute continuity of signed measures} 中的绝对连续性条件, 即存在一个函数 \(f_{\mathbf{X}}:{\mathbb{R}}^{n}\rightarrow\lbrack 0,\infty)\) 使得对于任意 Borel set \(B \subseteq {\mathbb{R}}^{n}\), 有
...
这个函数 \(f_{\mathbf{X}}\) 就是 continuous random vector \(\mathbf{X}\) 的 \textbf{joint probability density function (joint pdf).} 它是 \href{https://qiulinfan.github.io/notes/math/probability/\#kn-probability-density-function-pdf}{probability density function (pdf)} 在 random vector 上的对应概念. 它是 joint distribution measure \({\mathbb{P}}^{\mathbf{X}}\) 对于 Lebesgue measure \(\lambda^{n}\) 的 \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-radon-nikodym-derivative}{Radon-Nikodym derivative}:
...
显然是可以的. 因为 recall \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-fubini-theorem}{Fubini's Theorem}: 只要 \(f\) 绝对可积, 即 \(\left. \int_{{\mathbb{R}}^{n}} \middle| f \middle| \, d\lambda^{n} < \infty \right.\), 那么对于任意的 permutation \(\sigma\) of \(\left\{ {1,2,\cdots,n} \right\}\), 我们都可以将积分的顺序换成
...
\begin{proposition}{\protect\phantomsection\label{kn-joint-pdf-joint-cdf}{\textbf{joint pdf
和 joint cdf 的性质}}}
...
\item
  ...
  \pandocbounded{\includegraphics[keepaspectratio,alt={The shaded portion of the support lies below x+y=3/2, split at x=1/2.}]{assets/notes--fig-prob-03-joint-conditional-distribution-diagram-01.pdf}}
...
\begin{definition}{\protect\phantomsection\label{kn-independence-via-product-distribution}{\textbf{independence
via product distribution of marginal distributions}}}
...
这里离散情形沿用 \href{https://qiulinfan.github.io/notes/math/probability/\#kn-discrete-random-variable}{discrete random variable} 的定义. 详细而言:

\begin{theorem}{\protect\phantomsection\label{kn-independence-via-joint-density-factorization}{\textbf{independence
via joint-density factorization marginal densities}}}
...
Recall \href{https://qiulinfan.github.io/notes/math/probability/\#kn-independence-of-events}{independence of events} 的定义:
...
\begin{proposition}{\protect\phantomsection\label{kn-independence-via-conditional-distribution}{\textbf{independence
via conditional distribution: marginal distribution}}}
...
\begin{proposition}{\protect\phantomsection\label{kn-independence-via-generated-sigma-algebras}{\textbf{independence
via generated sigma-algebras}}}

令 \(X,Y\) 是两个 random variables. 则 \(X,Y\) 是 independent 的 iff: \(X\) 和 \(Y\) 按 \href{https://qiulinfan.github.io/notes/math/probability/\#kn-sigma-algebra-generated-by-a-random-variable-measurable-function}{\(\sigma\)-algebra generated by a random variable (measurable function)} 分别生成的 \(\sigma(X)\) 和 \(\sigma(Y)\) 中分别任取一个事件, 这两个事件都是 independent 的. 即:
...
\begin{definition}{\protect\phantomsection\label{kn-mutual-independence-of-multiple-random-variables}{\textbf{\textbf{mutual
...
\href{https://qiulinfan.github.io/notes/math/probability/\#kn-covariance}{covariance} 的定义:
...
我们称 covariance 为 0 的两个 random variables 为 \protect\phantomsection\label{kn-uncorrelated-random-variables}{\textbf{uncorrelated random variables}} . 我们容易发现: \textbf{independence 是一个比 uncorrelated 更强的概念:}

\begin{proposition}{\protect\phantomsection\label{kn-independent-uncorrelated}{\textbf{independent
严格强于 uncorrelated}}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Points constrained to a circle have zero covariance but remain dependent.}]{assets/notes--fig-prob-03-joint-conditional-distribution-diagram-02.pdf}}
...
\begin{theorem}{\protect\phantomsection\label{kn-independence-implies-expectation-is-closed-under-product}{\textbf{independence
\(\Longrightarrow\) expectation is closed under product}}}
...
\begin{proposition}{\protect\phantomsection\label{kn-discrete-rv-independence-characterization}{\textbf{discrete
RV independence 的 characterization}}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A rectangular joint support whose vertical cross-sections share one shape at different scales.}]{assets/notes--fig-prob-03-joint-conditional-distribution-diagram-03.pdf}}
...
\begin{definition}{\protect\phantomsection\label{kn-conditional-distribution}{\textbf{conditional
distribution}}}

给定 random variables \(X,Y:\Omega\rightarrow{\mathbb{R}}\), 其中下面极限里的条件概率按 \href{https://qiulinfan.github.io/notes/math/probability/\#kn-conditional-probability}{conditional probability} 理解. 我们定义 the conditional distribution of \(X\) given \(Y = y\) 为 the probability measure \({\mathbb{P}}^{X|Y = y}\):
...
\begin{theorem}{\protect\phantomsection\label{kn-jointly-continuous-rvs-defined-conditional-density}{\textbf{jointly
continuous RVs 之间所有 defined 处总有 conditional density}}}\label{thm-03-joint-conditional-distribution-jointly-continuous-rvs-defined-conditional-density}
...
\begin{definition}{\protect\phantomsection\label{kn-conditional-expectation}{\textbf{conditional
expectation}}}\label{def-03-joint-conditional-distribution-conditional-expectation}

令 \(X,Y:\Omega\rightarrow{\mathbb{R}}\) 为 RVs. 这里沿用 \href{https://qiulinfan.github.io/notes/math/probability/\#kn-expectation-and-variance-of-random-variable}{expectation and variance of random variable} 中 expectation 的积分定义. 对于 \(y \in {\mathbb{R}}\) where \(F_{X|Y} = {\mathbb{P}}^{X|Y = y}(X \leq x)\) is defined (这个条件对于 discrete 是筛选掉 \({\mathbb{P}}(y) = 0\) 的点, 对于 continuous 这是为了筛选掉 \(f_{Y} = 0\) 的点), 我们定义 conditional expectation:
...
\begin{proposition}{\protect\phantomsection\label{kn-independence-conditional-distribution-density-expectation}{\textbf{independence
下 conditional distribution 不变}} ,
\protect\phantomsection\label{kn-independence-conditional-density}{\textbf{independence
下 conditional density 不变}} 和
\protect\phantomsection\label{kn-independence-conditional-expectation}{\textbf{independence
下 conditional expectation 不变}}}
...
\begin{theorem}{\protect\phantomsection\label{kn-law-of-total-expectation}{\textbf{law
...
对于更加严格的 conditional expectation 的定义而言 (as an orthogonal projection from \(L^{2}(\Omega,\mathcal{F},{\mathbb{P}})\) onto the subspace of \(Y\)-measurable functions), 这个定理是 trivial 的, 直接 follow from def. 这个定义在 indicator function 上也包含 \href{https://qiulinfan.github.io/notes/math/probability/\#kn-kolmogorov-definition-of-conditional-probability}{Kolmogorov definition of conditional probability} 的情形. 在该定义中, \(\left. {\mathbb{E}}\lbrack X \middle| Y\rbrack \right.\) 被定义为一个 \(\sigma(Y)\)-measurable function \(Z\), 使得对于任意 \(A \in \sigma(Y)\), 都有
