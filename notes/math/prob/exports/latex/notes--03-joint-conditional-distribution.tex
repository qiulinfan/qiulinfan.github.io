...
recall: \href{https://qiulinfan.github.io/qlblog/notes/math/probability/\#kn-continuous-random-variable}{continuous random variable} \(X\) 的 cdf 是 absolutely continuous 的. 这个条件也等价于, 存在一个函数 \(f_{X}:{\mathbb{R}}\rightarrow\lbrack 0,\infty)\) 使得
...
对于 random vector \(\mathbf{X} = (X_{1},\cdots,X_{n})^{T}\) 如果 \({\mathbb{P}}^{\mathbf{X}} \ll \lambda^{n}\), 即它满足 \href{https://qiulinfan.github.io/qlblog/notes/math/measure-theory/\#kn-absolute-continuity-of-signed-measures}{absolute continuity of signed measures} 中的绝对连续性条件, 即存在一个函数 \(f_{\mathbf{X}}:{\mathbb{R}}^{n}\rightarrow\lbrack 0,\infty)\) 使得对于任意 Borel set \(B \subseteq {\mathbb{R}}^{n}\), 有
...
显然是可以的. 因为 recall \href{https://qiulinfan.github.io/qlblog/notes/math/measure-theory/\#kn-fubini-theorem}{Fubini's Theorem}: 只要 \(f\) 绝对可积, 即 \(\left. \int_{{\mathbb{R}}^{n}} \middle| f \middle| \, d\lambda^{n} < \infty \right.\), 那么对于任意的 permutation \(\sigma\) of \(\left\{ {1,2,\cdots,n} \right\}\), 我们都可以将积分的顺序换成
...
这里离散情形沿用 \href{https://qiulinfan.github.io/qlblog/notes/math/probability/\#kn-discrete-random-variable}{discrete random variable} 的定义. 详细而言:
...
Recall \href{https://qiulinfan.github.io/qlblog/notes/math/probability/\#kn-independence-of-events}{independence of events} 的定义:
...
\href{https://qiulinfan.github.io/qlblog/notes/math/probability/\#kn-covariance}{covariance} 的定义:
...
给定 random variables \(X,Y:\Omega\rightarrow{\mathbb{R}}\), 其中下面极限里的条件概率按 \href{https://qiulinfan.github.io/qlblog/notes/math/probability/\#kn-conditional-probability}{conditional probability} 理解. 我们定义 the conditional distribution of \(X\) given \(Y = y\) 为 the probability measure \({\mathbb{P}}^{X|Y = y}\):
