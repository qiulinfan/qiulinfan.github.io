...
如果 \(\Omega\) 是 \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-measure-space}{measure space} 意义下的一个 finite measure space, 那么对于任意 \(A_{1},\ldots,A_{n} \subseteq \Omega\) 的, 有:
...
按 \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-measure-space}{measure space} 的定义, 一个 probability space 是三元组 \((\Omega,\mathcal{F},{\mathbb{P}})\), 其中 \({\mathbb{P}}(\varnothing) = 0,{\mathbb{P}}(\Omega) = 1\).\\
对于这样的 \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-measure}{measure} \(\mathbb{P}\), 我们称之为 \textbf{probability measure (概率测度, 即概率)}.\\
而这里的 \(\Omega\) 我们称之为 \textbf{sample space (样本空间)}; 这里的 \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-sigma-algebra}{\(\sigma\)-algebra} \(\mathcal{F}\), 我们称之为 \textbf{event space (事件空间)}.\\
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A 60-by-60 arrival-time square with the band abs(x-y) \textless= 10 shaded.}]{assets/notes--fig-prob-01-combinatorics-prob-space-diagram-01.pdf}}
...
继承自原空间的 \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-sigma-algebra}{\(\sigma\)-algebra} \(\mathcal{F}\), 并被称为 \textbf{trace \(\sigma\)-algebra on \(B\)}.\\
...
当我们把整个 prob space 限制在 event \(B\) 上时, 本质上是在做一个 \href{https://qiulinfan.github.io/notes/math/measure-theory/\#kn-radon-nikodym-derivative}{Radon-Nikodym derivative} 的操作.\\
...
\item
  \({\mathbb{P}}(A \mid \mathcal{G})\) 实际就是 \href{https://qiulinfan.github.io/notes/math/probability/\#kn-conditional-expectation}{conditional expectation} \({\mathbb{E}}\lbrack{\mathbb{I}}_{A} \mid \mathcal{G}\rbrack\). 当 \({\mathbb{P}}(B) > 0\) 时, 它就退化回经典定义 \(\frac{{\mathbb{P}}(A \cap B)}{{\mathbb{P}}(B)}\).
\end{itemize}
