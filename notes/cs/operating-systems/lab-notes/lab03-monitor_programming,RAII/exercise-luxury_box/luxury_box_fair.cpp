...
void pac12_wants_enter(uintptr_t) {
    box_lock.lock();
    waiting_pac12++;
    while (active_big10 > 0 || (TURN == BIG10 && waiting_big10 > 0)) {
...
void pac12_leave(uintptr_t) {
    box_lock.lock();
    active_pac12--;
    // pac12 party already got the room before, so now we prioritize BIG10
    TURN = BIG10; // Allow BIG10 to enter next
...
void big10_leave(uintptr_t) {
    box_lock.lock();
    active_big10--;
    // big10 party already got the room before, so now we prioritize PAC12
    TURN = PAC12; // Allow PAC12 to enter next
